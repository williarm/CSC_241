...
public class WeightedGraph<T> implements WeightedGraphInterface<T> {
	...
	// Marks vertex. indicates the vertex has been visited
	{
	...
	public String toString() {
		// Print the connections and edges for each vertex
		String string = "";
		for (int i = 0; i < vertices.length; i++) {
			if (vertices[i] != null) {
				string+= "\r\n";
				...
				for (int j = 0; j < vertices.length; j++) {
					try {
						if (edgeExists(vertices[i], vertices[j])) {
							string += vertices[j] + ", " + weightIs(vertices[i], vertices[j]) + "\r";
						}
