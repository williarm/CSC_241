...
public class BowlingGame {
	...
	public void computeScoreFrame() {
		...
		if (f >= 2) { // look two frames back
			...
			if (scores[f - 1][0] == 10 && scores[f - 2][0] == 10) { //strikes last 2 frames
				if (f == 9) {
					score += ((scores[f][0] * 2 + scores[f][1]) + (scores[f][0] + scores[f][1] + scores[f][2]));
				} else {
					score += ((scores[f][0] * 2 + scores[f][1]) + (scores[f][0] + scores[f][1]));
				}
				System.out.println("f>=2, 2 previous frames had strikes. frame: " + (f + 1) + " score: " + score);

			} else if (scores[f - 1][0] == 10) { //strike last frame
				if (f == 9) {
					score += ((scores[f][0] + scores[f][1]) * 2 + scores[f][2]);
				} else {
					score += ((scores[f][0] + scores[f][1]) * 2);
					System.out.println("f>=2, 1 previous frame had a strike. frame: " + (f + 1) + " score: " + score);
				}

			} else if (scores[f - 1][0] + scores[f - 1][1] == 10) { //spare last frame
				if (f == 9) {
					score += ((scores[f][0] + scores[f][1]) * 2 + scores[f][2]);
				} else {
					score += (scores[f][0] * 2) + scores[f][1];
				}
				System.out.println("f>=2, 1 previous frame had a spare. frame: " + (f + 1) + " score: " + score);
				...
				System.out.println("frame: "+ (f+1) + " score: " + score);
			}
		} else if (f == 1) { // look one frame back
			// your algorithm for the bonus points goes here.
			if (scores[f - 1][0] == 10) {
				score += ((scores[f][0] + scores[f][1]) * 2);
				System.out.println("previous frame had a strike. frame: " + (f + 1) + " score: " + score);
			} else if (scores[f - 1][0] + scores[f - 1][1] == 10) {
				score += (scores[f][0] * 2) + scores[f][1];
				System.out.println("previous frame had a spare. frame: " + (f + 1) + " score: " + score);
				// System.out.println("score: "+score);
				// System.out.println("scores[f][0]: "+scores[f][0]);
			...
			score += scores[f][0] + scores[f][1];
			System.out.println("first frame score: " + score);
		}
